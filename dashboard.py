...
@st.cache_data
def load_data():
    prices = pd.read_csv(BASE_DIR / "crop_prices.csv")
    yields = pd.read_csv(BASE_DIR / "crop_yield.csv")
    with open(BASE_DIR / "lease_rates.json") as f:  lr = json.load(f)
    with open(BASE_DIR / "district_lease_rates.json") as f:  dr = json.load(f)
